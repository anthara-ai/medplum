...
A markdown file at `docs/specs/<NNN>-<slug>-context.md`, where `<NNN>` and `<slug>` come from the spec's filename. The file has eight numbered sections plus three per-slice sub-sections (§4a imitation targets, §4b lexicon, §4c traps; see canonical structure below). Aim for **80-200 lines total** when the spec has up to ~10 slices; scale linearly above that. Prefer aggregation over enumeration when over-budget. A slice with no applicable imitation target, no fixed lexicon, or no foreseeable traps may be omitted from the respective sub-section — do not pad to satisfy a template.
...
5. **Pick one imitation target per slice.** For each slice that will write a new function, identify the *single* most adjacent existing function whose shape (argument order, error handling, return type, projection, naming style) should be mirrored. Greenfield modules with no adjacent example: say so explicitly; do not invent one. The developer agent reads this to anchor shape decisions.
6. **Enumerate slice lexicon.** For each slice, list the load-bearing vocabulary the developer must not invent or drift: verb names (e.g., `dispatchBot` — not `triggerBot` or `startDispatch`), column names, audit-event strings, query-key shapes, route paths. Pin only names that the slice can be expected to fix in code — do not list every identifier the slice touches. A typical slice has 3-8 lexicon entries.
7. **List slice traps.** For each slice, enumerate 2-5 ways a competent-but-non-thinking executor would implement it wrong. Source from: spec's resolved annotations, dragons in adjacent modules, invariants in §4 of the spec, recurring mistakes from prior `[PATTERN]` records on fabric if reachable. Each trap is one line, action-shaped (e.g., *"do NOT update updated_at in the action body — the trigger does it"*).
8. **Identify dragons.** Search for `TODO`, `FIXME`, `DEPRECATED`, `LEGACY`, `frozen`, race-condition comments. Surface compliance-locked code, areas with named owners, places where modification is gated.
9. **Note test conventions.** Where tests live (co-located? separate?), test framework, fixtures (especially RLS or three-user fixtures for HIPAA contexts), naming patterns.
10. **Identify hot paths and performance landmines.** Endpoints flagged as latency-sensitive in code comments or docs; queries that should not gain joins; loops that should not gain bounds.
11. **Note open uncertainties resolved during orientation.** If exploring revealed an answer to one of the spec's §11 open questions (e.g., "build vs integrate" — confirmed integrating with existing `dispatch.service.ts`), capture that.
...
- **Slice 5.X (<name>):** mirror `<file>` (`<symbol>`) — same arg order, error mapping, return type, projection.
- **Slice 5.Y (<name>):** greenfield module; no imitation target. Establish shape carefully.

## 4b. Slice lexicon
- **Slice 5.X:** verb=`<dispatchBot>`; column=`<dispatch_status>`; event=`<bot.dispatched>`; route=`<POST /bots/dispatch>`; query key=`<['meeting-bots', meetingId]>`.
- **Slice 5.Y:** verb=`<linkPullRequest>`; event=`<pr.linked>`.

## 4c. Slice traps
- **Slice 5.X:** (1) do NOT bypass `audit.logEvent` on the dispatch path. (2) do NOT call the Recall SDK directly — use `recall.adapter`. (3) `.select(...)` projection must exclude `details->>'phi'`.
- **Slice 5.Y:** (1) verify the PR URL host before storing — SSRF risk per spec invariant 4.X.
...
After writing the context file, return a brief summary to the orchestrator: *"Generated context at docs/specs/<NNN>-<slug>-context.md. N modules walked, M patterns identified, K reusables, J dragons flagged. Per-slice intelligence: I imitation targets, L lexicons, T traps across S slices."*
