...
- **Test failure** — per `test-execution`: scoped one-shot of the specific failing test file (`pnpm vitest run <file>`, `pytest <file> -q`, or the project's equivalent — never the whole suite).
...
- Re-check the failing test — per `test-execution`, scoped one-shot of the specific test file after each candidate fix.
- Run the related test suite (broader than the single test, narrower than full suite — covering the modules the fix touches) as a scoped one-shot for verification.
