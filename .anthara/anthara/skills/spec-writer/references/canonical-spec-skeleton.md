...
- **Reachability:** <Host> → <Entry>.

  One line per path. The left side names a Host — either an earlier slice's Entry, verbatim, or `root` for the legitimate first surfaces (the app shell, the CLI entry point, the public API root). The right side names this slice's Entry — the outermost user-reachable thing this slice exposes (a screen, button, CLI subcommand, HTTP endpoint, webhook subscriber, scheduled job). When the same Entry is reachable from multiple Hosts (e.g. a "+ File bug" button on both the board and the backlog), write one Reachability line per Host. A slice may not be ordered before every Host it names has been created by an earlier slice.

  Examples:

  - `root → the project shell at /<org>/<project>` (a first slice).
  - `the product detail screen (5.3) → "Add to cart" button` (single Host).
  - `the board screen (5.12) → "+ File bug" button` and `the backlog screen (5.16) → "+ File bug" button` (two lines, same Entry).
...
2. **Migrations.** Apply all migrations against the project's database; they apply without error. Database state is not assumed reset.
3. **User-facing flow.** Drive the slice's primary user-observable behavior end-to-end through the `chrome-devtools-mcp` browser session — confirm what this slice's outside-in description and ACs promise actually happens in the running app.
4. **Accessibility.** In the same `chrome-devtools-mcp` session: tab-order traversal, focus-visibility on every interactive control, aria-labels present per the project's design system.

(Conditional inclusion: omit step 2 when the slice doesn't change schema; omit steps 3 and 4 when the slice has no UI surface — e.g. pure backend or MCP-only. Step 1 always present.)
